...
(function (global) {
    ...
    const FILE_REFS = {
        ...
        'crates/node/src/shard/mod.rs': { lineHint: 'ShardLoop::step, ShardScopedInput dispatch', note: 'Tx ingress, overview, E2E (was shard_loop)' },
        'crates/node/src/shard/protocol_event.rs': { lineHint: 'feed_event continuations after persist', note: 'Messaging inventory' },
        'crates/node/src/shard/mempool/validation.rs': { lineHint: 'SubmitTransaction → TransactionValidated', note: 'Tx ingress' },
        'crates/node/src/state/mod.rs': { lineHint: 'NodeStateMachine::handle, shard + beacon coordinators', note: 'Tx flow, overview' },
        'crates/node/src/state/participation/transactions.rs': { lineHint: 'ProtocolEvent::TransactionValidated → mempool', note: 'Tx flow' },
        'crates/node/src/state/participation/proposal.rs': { lineHint: 'gather_proposal_inputs, try_event_driven_proposal', note: 'Shard proposal retry' },
        'crates/node/src/state/participation/shard.rs': { lineHint: 'handle_shard, on_qc_formed, on_block_committed order', note: 'Node shard dispatch' },
        'crates/node/src/state/beacon.rs': { lineHint: 'handle_beacon, BeaconCoordinator dispatch', note: 'Beacon chain events' },
        'crates/node/src/state/participation/timers.rs': { lineHint: 'ViewChangeTimer, CleanupTimer, shard_coordinator.check_round_timeout', note: 'Timing module-09' },
        'crates/node/src/shard/io.rs': { lineHint: 'ShardIo — fetch/sync/block serve protocol wiring', note: 'Overview flow 1' },
        'crates/node/src/process/network_handlers.rs': { lineHint: 'register_* handlers → ShardScopedInput', note: 'Tx ingress, messaging inventory' },
        'crates/node/src/process/mod.rs': { lineHint: 'ProcessIo, shared process state', note: 'Production ingress' },
        'crates/node/src/host.rs': { lineHint: 'NodeHost, SubmitTransaction fan-out, register_inbound_handlers', note: 'Production ingress' },
        'crates/node/src/shard/actions.rs': { lineHint: 'Action::BuildProposal, ExecuteCrossShardTransactions dispatch', note: 'Shard build + execution' },
        ...
        'crates/types/src/topology/schedule.rs': { lineHint: 'TopologySchedule::at(parent_qc.wt), head, routing_committees — parent-anchored committee windows', note: 'Cross-shard Phase 3–4; verification vs routing; Phase 2 proposer committee' },
        'crates/types/src/topology/snapshot.rs': { lineHint: 'TopologySnapshot::proposer_for(shard, round) — round % committee.len()', note: 'Phase 2 proposer selection; cross-shard Phase 4 routing' },
        ...
        'crates/provisions/src/coordinator.rs': { lineHint: 'ProvisionCoordinator, source_block_ts from parent_qc, orphan cleanup_orphans', note: 'Cross-shard Phase 4' },
        ...
        'crates/types/src/shard/roots/reveal_chain.rs': { lineHint: 'RevealChain — epoch-scoped VRF reveal hash chain', note: 'Header reveal_chain; beacon randomness fold' },
        'crates/types/src/shard/witness_sources.rs': { lineHint: 'WitnessSources: ready_signals, reshape_trigger, randomness_reveal', note: 'Block::Live body; feeds beacon witness + reveal_chain' },
        'crates/types/src/shard/roots/beacon_witness.rs': { lineHint: 'derive witness leaves, merkle range multiproofs', note: 'Beacon witness window + reshape leaf classification' },
        'crates/simulation/tests/scenarios.rs': { lineHint: 'single_shard_tx_sim, cross_shard_tx_sim via SimCluster', note: 'E2E harness labs' },
        'crates/simulation/tests/support/sim_cluster.rs': { lineHint: 'SimCluster adaptor over SimulationRunner', note: 'E2E harness labs' },
        'crates/production/tests/runner.rs': { lineHint: 'runner_boots_listens_and_shuts_down_cleanly', note: 'Production E2E lifecycle' },
        'crates/production/tests/support/harness.rs': { lineHint: 'build_runner, TestFixtures wiring', note: 'Production E2E harness' },
    };
