...
const GLOSSARY_ENTRIES = [
  ...
  },
  {
    ...
    technicalDef: 'In leader-based BFT, each view (round) has one leader. If the leader stalls (no QC, no useful proposal), validators advance the round and pick the next proposer via deterministic round-robin (`proposer_for(height, round)`). In PBFT this uses coordinated view-change votes; HotStuff-style protocols use **implicit view change**: `ViewChangeTimer` fires → `check_round_timeout` bumps the round → a new leader may `try_propose`. Hyperscale-rs arms `TimerId::ViewChange` at genesis; there is no separate “view-change message” phase in the hot path—liveness comes from timeout + new round, not from a periodic proposal timer.',
  ...
  },
  {
    ...
    technicalDef: 'In hyperscale-rs, `BftCoordinator::queue_ready_proposal()` sets `verification.ready_proposal = true` (a latch, not a queue of txs). After `NodeStateMachine::handle` finishes processing an event, `take_ready_proposal()` clears it; if true, `try_event_driven_proposal` gathers mempool/execution inputs and calls `try_propose`. It does not store *what* changed—only that a proposal attempt may be worthwhile. Distinct from mempool storage or crossbeam channels.',
  ...
  },
  {
    ...
    technicalDef: 'In hyperscale-rs, `ProductionRunner::run` (Tokio side) waits on a metrics tick while the pinned `run_pinned_loop` runs. Each tick refreshes gauges and histograms installed at startup (`hyperscale_metrics_prometheus` and related). That gives steady samples for dashboards and alerts; trade-off: a small periodic wake vs event-only sampling, which can hide “stuck but idle” or slowly growing backlog.',
  ...
  },
  {
    ...
    technicalDef: 'After handling an event, production code can record each channel’s pending length (bounded queues expose depth cheaply). Rising depths mean producers (network, Tokio timers, RPC) are enqueueing faster than `IoLoop::step` drains—often a precursor to latency spikes or timeouts. Export is O(channels) and does not require tracing; trade-off: depth alone does not show message size or work per message.',
  ...
  },
  {
    ...
    keys: ['block commit proof', 'block commit', 'finality proof'],
  ...
  },
  {
    ...
    def: 'A small instruction from the IoLoop telling production how to arm or cancel a logical timer; processed by ProdTimerManager using Tokio sleeps.',
    technicalDef: 'When the state machine handles genesis or normal steps, the IoLoop can emit `TimerOp`s (e.g. `Set { id, duration }` to schedule, or cancel). In production, `ProdTimerManager::process_op` runs on the pinned thread but uses the Tokio handle to `spawn` an async `sleep`; when it fires, it sends a timer-scoped `NodeInput` on the crossbeam `timer_rx` channel (highest priority in `run_pinned_loop`). `initial_timer_ops` are timer ops produced during genesis initialization on the runner thread and drained once at the very start of `run_pinned_loop` so the proposal timer (and any other genesis-time timers) exist before the main event loop.',
    ...
    competingConcepts: 'TimerOp bridges synchronous `IoLoop` logic with async Tokio timers; simulation may use a single simulated clock instead. Cancelling a timer replaces the previous Tokio task for that `TimerId`.',
  },
  {
    key: 'state root',
