#!/usr/bin/env node
/** Regenerate hyperscale course-data.js — 7 levels, 21 modules (L6: JMT + cross-shard labs + performance). */
const fs = require('fs');
...
const modules = [
    ...
    { courseLevel: 3, id: 'hs-phase-02', title: 'Deep dive — Phase 2: Propose → Commit', duration: '1.5–2 hours', difficulty: 'Level 3', path: 'hyperscale/hyperscale-rs/module-phase-02-propose-vote-commit.html', description: 'In-depth drill on propose → vote → QC → commit; shard coordinator paths and in-repo proofs.', objectives: ['Explain proposer_for(shard, round) vs can_propose and why there is no ProposalTimer', 'Trace BuildProposal → ProposalBuilt → vote → QC → commit_block', 'Separate revote from equivocation and QC-via-parent_qc from “QC gossip”'], hyperscaleSpecific: true, contributionModule: false },
    { courseLevel: 3, id: 'hs-phase-03', title: 'Deep dive — Phase 3: Execution & Waves', duration: '1.5–2 hours', difficulty: 'Level 3', path: 'hyperscale/hyperscale-rs/module-phase-03-execution-waves.html', description: 'Post-commit execution: waves, EC aggregation, finalize_wave → FinalizedWavesAdmitted; includes Hyperscale-rs vs Radix Engine boundary.', objectives: ['Trace ExecutionCoordinator::on_block_committed and setup_waves_and_dispatch', 'Separate commit time from Radix execution and FinalizedWave inclusion', 'Explain wave leader, WAVE_TIMEOUT, and where the external engine runs manifests'], hyperscaleSpecific: true, contributionModule: false },
    { courseLevel: 4, id: 'hs-phase-04', title: 'Deep dive — Phase 4: Cross-Shard Tx & Provisions', duration: '2–3 hours', difficulty: 'Level 4', path: 'hyperscale/hyperscale-rs/module-phase-04-cross-shard-tx.html', description: 'Unified module: 20-step end-to-end flow, provisions deep dive, block fields map, Merkle/JMT why, 33-item Q&A, USDC reference scenario — provision-based cross-shard (no 2PC).', objectives: ['Trace submit → provisions@H → ProvisionedSnapshot → FinalizedWave', 'Distinguish provision_tx_roots, provision_root, and JMT proofs', 'Explain shadow base, anchor height H, and manifest vs protocol ordering'], hyperscaleSpecific: true, contributionModule: false },
    { courseLevel: 4, id: 'hs-block-fields', title: 'Deep Dive: Block Fields & Root Commitments', duration: '35–50 min', difficulty: 'Level 4', path: 'hyperscale/hyperscale-rs/module-block-fields-deep-dive.html', description: 'After Phases 1–4: commitments vs signatures, how each *_root is computed, per-block scope, WaveCertificate vs teaching “transaction certificate”.', objectives: ['Distinguish consensus (QC) vs execution (FinalizedWave) vs cross-shard (provisions)', 'Compute transaction_root and explain state_root JMT replay', 'Trace one tx through roots without a global history Merkle'], hyperscaleSpecific: true, contributionModule: false },
    { courseLevel: 4, id: 'hs-timers', title: 'Timers & Chain Time', duration: '40–55 min', difficulty: 'Level 4', path: 'hyperscale/hyperscale-rs/module-timers-reference.html', description: 'Master table of every timer/timeout—purpose in English, chain time vs wall clock, event-driven proposals.', objectives: ['Explain height vs round and ViewChange liveness', 'Map WAVE_TIMEOUT, provision fallback, and fetch timeouts to their clocks', 'Contrast proposal latch with ViewChangeTimer'], hyperscaleSpecific: true, contributionModule: false },
    { courseLevel: 5, id: 'hs-e2e-harness', title: 'E2E harnesses (simulation + production)', duration: '2–2.5 hours', difficulty: 'Level 5', path: 'hyperscale/hyperscale-rs/module-hs-simulation-harness-analysis.html', description: 'SimCluster/EventKey plus production pinned shard-loop, RPC ingress, and observability—reading before hands-on labs.', objectives: ['Separate sim time from wall clock and trace single_shard_tx_sim', 'Map RPC SubmitTransaction vs tx_submission_sender to consensus_rx', 'Contrast SimCluster with run_shard_loop production wiring'], hyperscaleSpecific: true, contributionModule: false },
    { courseLevel: 5, id: 'hs-improved-sim-tests', kind: 'project', title: 'Single-Shard Simulation E2E Tests: Improved/Innovative New Test Case', duration: '1.5–2 hours', difficulty: 'Level 5', path: 'hyperscale/hyperscale-rs/module-hs-improved-simulation-tests.html', description: 'Hands-on: instrument single_shard_tx_sim and explore SimulationStats vs simulated time.', objectives: ['Add trace markers to an existing single-shard simulation E2E test', 'Assert height synchronization with harness-aware tolerance', 'Explain why starving simulated time blocks progress'], hyperscaleSpecific: true, contributionModule: false },
    { courseLevel: 5, id: 'hs-improved-prod-tests', kind: 'project', title: 'Single-Shard Production E2E Tests: Improved/Innovative New Test Case', duration: '1.5–2 hours', difficulty: 'Level 5', path: 'hyperscale/hyperscale-rs/module-hs-improved-production-tests.html', description: 'Hands-on: instrument runner_boots_listens_and_shuts_down_cleanly and document ingress hooks.', objectives: ['Add tracing markers to a production integration test', 'Assert network listen addresses and clean shutdown', 'Document how you would inject SubmitTransaction via tx_submission_sender'], hyperscaleSpecific: true, contributionModule: false },
    { courseLevel: 6, id: 'hs-jmt-deep-dive', title: 'Deep Dive: JMT & Shard State Storage', duration: '50–70 min', difficulty: 'Level 6', path: 'hyperscale/hyperscale-rs/module-jmt-deep-dive.html', description: 'After E2E labs: one JMT per shard, RocksDB vs tree layers, owner-prefixed jmt_leaf_key, genesis insert vs transfer update — with in-repo tests and prepare_block_commit.', objectives: ['Explain entity + partition + sort_key and owner-major JMT leaf keys (state_key.rs)', 'Trace genesis install and insert vs update on the same storage_key', 'Relate execution database_updates to state_root on a later block header'], hyperscaleSpecific: true, contributionModule: false },
    { courseLevel: 6, id: 'hs-improved-cross-shard-sim-tests', kind: 'project', title: 'Cross-Shard Simulation E2E Tests: Improved/Innovative New Test Case', duration: '1.5–2 hours', difficulty: 'Level 6', path: 'hyperscale/hyperscale-rs/module-hs-improved-cross-shard-simulation-tests.html', description: 'Hands-on: instrument cross_shard_tx_sim (multi_shard_config, six validators).', objectives: ['Run and trace the deterministic cross-shard simulation E2E', 'Add markers around genesis funding and cross-shard submit', 'Compare mempool and finalization signals on both shard groups'], hyperscaleSpecific: true, contributionModule: false },
...
const inter = modules.filter((m) => m.courseLevel >= 2 && m.courseLevel <= 6);
const adv = modules.filter((m) => m.courseLevel === 7);

const head = `// Course data — Levels 1–7 (21 modules): L6 has four modules (JMT deep dive + cross-shard labs + performance); L7 has two specialization modules.
