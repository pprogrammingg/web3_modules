...
const GLOSSARY_ENTRIES = [
  ...
  },
  {
    ...
    explain10yo: 'When two clubhouses can\'t talk to each other, you either keep their lists exactly the same (but sometimes you can\'t answer "what\'s the list?") or you always answer (but the two lists might not match).',
  ...
  },
  {
    ...
    def: 'The ability of a system to function correctly even when some nodes are Byzantine (malicious or faulty).',
    technicalDef: 'In a BFT system with n = 3f+1 nodes, up to f nodes can be arbitrary (Byzantine): they may crash, delay, or send conflicting messages. The protocol still guarantees safety (all correct nodes agree) and liveness (progress) as long as at most f nodes are faulty.',
    explain10yo: 'The game still works even if some players try to cheat or give wrong answers, as long as most players are honest and follow the rules.',
    subCategory: ['Consensus & Agreement', 'Fault Models & Security'],
    competingConcepts: 'BFT vs crash fault tolerance (CFT): BFT handles malicious nodes but needs 3f+1 nodes and more rounds; CFT (e.g. Raft) needs only 2f+1 and is simpler. BFT vs Nakamoto: BFT has fast deterministic finality; Nakamoto uses PoW/PoS and probabilistic finality but no strict identity/sync assumptions.',
  },
  {
    key: 'pbft',
    keys: ['pbft'],
  ...
  },
  {
    ...
    def: 'Consensus mechanism where nodes compete to solve a costly puzzle; solution grants the right to propose a block.',
    technicalDef: 'Miners repeatedly hash block candidates with a nonce until the hash meets a target (e.g. leading zeros). Finding such a nonce is hard; verifying is easy. The solver proposes the next block and is rewarded. Security comes from the cost of amassing majority hashrate.',
    explain10yo: 'A race where you have to solve a hard math puzzle. The first to solve it gets to write the next page. It\'s hard to do but easy for everyone to check.',
    subCategory: ['Consensus & Agreement', 'Economics & Incentives'],
    competingConcepts: 'PoW vs PoS: PoW uses energy and hardware; PoS uses staked capital. PoW is battle-tested and permissionless; PoS avoids energy cost and can have faster finality but introduces stake concentration and "nothing at stake" style issues that protocols must address.',
  },
  {
    key: 'proof of stake',
