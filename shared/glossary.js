...
const GLOSSARY_ENTRIES = [
  ...
  },
  {
    ...
    technicalDef: 'Block at height H commits once H has a QC and the block at H+1 also has a QC. The chain of two QCs proves that a quorum has agreed on H and then on H+1, so H is safe. Used in HotStuff and similar protocols. Often called the two-chain rule or two-chain commit.',
  ...
  },
  {
    ...
    technicalDef: 'Livelock (general): Unlike deadlock (everyone blocked waiting), in a livelock everyone is "active" but the system doesn\'t move forward—e.g. two shards repeatedly deferring each other\'s transactions. In the hyperscale-rs codebase: the livelock crate handles cross-shard dependency cycles. Two (or more) shards can each commit a transaction that depends on state from the other (e.g. Shard A commits TX₁ needing state from C; Shard C commits TX₂ needing state from A → a cycle). When a shard gets a provision (proof that another shard committed a cross-shard tx), it runs cycle detection. If both have committed txs that need the other\'s state, that\'s a cycle. To break it, one transaction is deferred (e.g. the one with the larger hash "loses" and is retried later). Both shards use the same deterministic rule so they agree without extra messages. The --analyze-livelocks flag in the simulator reports how often such situations occurred in a run.',
    ...
    competingConcepts: 'Livelock vs deadlock: deadlock = everyone blocked waiting (no one runs); livelock = everyone active but no progress (e.g. retrying forever). In hyperscale-rs, "livelock" effectively means a cycle of cross-shard dependencies plus the logic that detects and breaks it by deferring one tx deterministically.',
  },
  {
    key: 'rollup',
