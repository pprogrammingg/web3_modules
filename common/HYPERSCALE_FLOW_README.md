...
3. **See what to re-check:** Run **`node scripts/check-hyperscale-changes.js`** (no args). It diffs the **local** hyperscale-rs repo against the stored baseline in **`common/hyperscale-rs-last-synced.txt`** (or `main` if the file is missing). Re-check the listed modules against the local repo.
