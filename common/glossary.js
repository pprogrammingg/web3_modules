...
const GLOSSARY_ENTRIES = [
  ...
  },
  {
    ...
    technicalDef: 'After handling an event, production code can record each channel’s pending length (bounded queues expose depth cheaply). Rising depths mean producers (network, Tokio timers, RPC) are enqueueing faster than `ShardLoop::run_step` drains—often a precursor to latency spikes or timeouts. Export is O(channels) and does not require tracing; trade-off: depth alone does not show message size or work per message.',
  ...
  },
  {
    ...
    technicalDef: 'Hyperscale-rs `MempoolCoordinator` keeps a `TombstoneStore`: `tx_hash → validity end time`. When a tx reaches a terminal status (e.g. `Completed`, `Aborted`), the pool entry is removed and the hash is tombstoned so gossip or RPC cannot put it back in `Pending`. `ShardLoop` skips validation enqueue if `is_tombstoned(&hash)` (see `handle_gossip_received_tx_for_validation` in `shard_loop/step/tx_validation.rs`). Tombstones are pruned when chain time (`WeightedTimestamp` from committed QCs) passes the tx\'s `validity_range.end_timestamp_exclusive`; matching bodies may then leave `TxStore`. This is per-validator dedup policy—not an on-chain tombstone object.',
  ...
  },
  {
    ...
    technicalDef: 'Per remote `ShardId`, merkle of tx hashes in this block that touch that shard. Targets verify incoming Provisions batches cover the committed set.',
  ...
  },
  {
    ...
    alsoRelevant: 'Witness window — **ready_signals** in block body; **beacon chain**.',
  ...
  },
  {
    ...
    alsoRelevant: 'Reshape — **reshape_trigger** in block body; beacon split/merge.',
  ...
  {
    key: 'block-field-body-ready-signals',
    keys: ['block-field-body-ready-signals'],
    term: 'Block body.ready_signals',
    def: 'BLS-signed ready signals the proposer drained into this block for the beacon witness.',
    technicalDef: 'Committed via `beacon_witness_root`. Carried on the block so commit-time witness leaf derivation is byte-identical across validators, sync reloads, and re-proposals.',
    alsoRelevant: 'Witness base — **beacon_witness_base**; **beacon chain**.',
    explain10yo: 'Signed “we’re ready” notes stapled into the page for the league roster.',
    blockFieldMeta: { phaseSteps: 'Beacon + Phase 2' },
    subCategory: ['Block header & body'],
  },
  {
    key: 'block-field-body-reshape-trigger',
    keys: ['block-field-body-reshape-trigger'],
    term: 'Block body.reshape_trigger',
    def: 'Optional split/merge assertion for this block (`None` on normal blocks).',
    technicalDef: 'Committed via `beacon_witness_root` alongside `ready_signals`. Beacon reshape scheduling reads the substate bytes carried here.',
    alsoRelevant: 'Terminal headers — **split_child_roots**, **settled_waves_root**.',
    explain10yo: 'A note on the page saying “this team is splitting” or “these two teams are merging.”',
    blockFieldMeta: { phaseSteps: 'Beacon reshape (advanced)' },
    subCategory: ['Block header & body', 'Scalability & Sharding'],
  },
];
